...
import models.deepCLife as deepCLife
import models.SourceOnly as SourceOnly
...
def training_soft_survival(datasets_groups):
    ...
    elif 'deepCLife' in args.mode:
        classifier, optimizer = deepCLife.make_models(
            layer_params, args.learning_rate
        )
    ...
    for epoch in range(args.epoch_num):
        ...
        for step in range(step_num):
            ...
            if 'SRPS' in args.mode:
                ...
                loss_su, loss_surv, loss_l1 = deepCLife.train_step(
                    x_source, y_source, x_target, OS, status, DFS, recurrence,
                    classifier, 
                    optimizer,
                    [args.loss_su, args.loss_survival],
...
def training_deepCLife(datasets_groups):
    datasets_train, datasets_valid, datasets_test = datasets_groups
    ...
    test_saved_r = args.initial_valid_best_r
    for epoch in range(args.epoch_num):
        x1_list, y1_list, x2_list, OS_list, status_list, DFS_list, recurrence_list = [], [], [], [], [], [], []
        r_list = []
        for dataset_id, dataset in enumerate(datasets_train):
            sample_num = len(dataset['OS'])
            ids = np.random.choice(
                np.arange(sample_num), int(sample_num / 2), replace=False
            ) if args.mini_batch else np.arange(sample_num)
            x = dataset['data'].astype(np.float32)[ids, :]
            OS = dataset['OS'][ids]
            status = dataset['status'][ids]
            DFS = dataset['DFS'][ids]
            recurrence = dataset['recurrence'][ids]

            a = classifier.predict(x)
            if dataset_id == 0:
                y1 = dataset['subtypes'].astype(np.int32)[ids]
                x1_list.append(x)
                y1_list.append(y1)
                acc = np.mean(a == y1)
            else:
                ...
                r = (data_utils.get_reward(OS, status, a, args.subtype_num) + 
                    data_utils.get_reward(DFS, recurrence, a, args.subtype_num)
                )/2
                r_list.append(r[0])
        ...
        valid_r_list = []
        for dataset_id, dataset in enumerate(datasets_valid):
            x = dataset['data'].astype(np.float32)
            OS = dataset['OS']
            status = dataset['status']
            DFS = dataset['DFS']
            ...
            if dataset_id == 0:
                valid_acc = np.mean(valid_a==dataset['subtypes'])
            else:
                r_valid = (data_utils.get_reward(OS, status, valid_a, args.subtype_num) + 
                    data_utils.get_reward(DFS, recurrence, valid_a, args.subtype_num)
                )/2
        ...
        if valid_best_r < valid_mean_r and epoch > 100:
            classifier.save_weights(model_path)
            valid_best_r = copy.deepcopy(valid_mean_r)
            test_saved_r = copy.deepcopy(test_mean_r)
            print('save model')
        ...
        # trainig model
        training_start_time = time.time()
        loss_su, loss_sv, loss_l1 = deepCLife.train_step(
            x1_list, y1_list, x2_list, OS_list, status_list, DFS_list, recurrence_list,
            classifier, 
            optimizer, 
            [args.loss_su, args.loss_survival],
        ...
        print('epoch: {:} |'.format(epoch) +
            'T: {:.1f} |'.format((time.time() - start_time)/60.) +
            'l_su: {:.2f} |'.format(loss_su) +
            'l_sv: {:.2f} |'.format(np.fabs(loss_sv)) +
            'l_l1: {:.3f} |'.format(loss_l1) +
            'acc: {:.2f} |'.format(acc) +
            'r: {:.2f} |'.format(mean_r) +
            'v_acc: {:.2f} |'.format(valid_acc) +
...
def training_DA(datasets_groups):
    ...
    if 'SourceOnly' in args.mode:
        args.learning_rate = 0.01
        args.epoch_num = 2000
        classifier, optimizer = SourceOnly.make_models(args.learning_rate, layer_params)
    ...
    for epoch in range(args.epoch_num):
        ...
        for step in range(step_num):
            ...
            # optimization
            if 'SourceOnly' in args.mode:
                loss_su = SourceOnly.train_step(
                    x_source, y_source, x_target,
                    classifier, 
                    optimizer,
                    args.dropout, 
                    tf.cast(args.learning_rate, dtype=tf.float32)
                )
                if np.isnan(loss_su.numpy()):
                    nan_flag = True
                    break
            elif 'DANN' in args.mode:
